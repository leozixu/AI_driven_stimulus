#!/usr/bin/env python3
"""
spi_master_public - harness.py
Black-box eval loop for the SPI master serial transfer controller.
...
(SPEC_CFS_MIN, SPEC_HOLD_SS, SPEC_TXFTLR_DFLT) into local_sim and the coverage
simulator, samples the coverage curve every `interval` cycles.
...
Action layout (12 dims, matches dut_spec.md §6):
  ...
  - reg_addr   (0..63)        : 6-bit word address (see dut_spec.md reg map)
...
def default_secrets() -> Dict:
    return {
        "spec_cfs_min": 8,     # minimum Microwire control frame size (clamp)
...
def load_secrets(path: Optional[str] = None) -> Dict:
    if path is None:
        cands = [
            os.path.join(HERE, "..", "..", "committee_eval_private", "eval_secrets_spi_master.json"),
            os.path.join(HERE, "..", "..", "..", "发布包_v1.4", "committee_private",
                         "committee_eval_private", "eval_secrets_spi_master.json"),
        ]
        for c in cands:
            if os.path.exists(c):
                path = c
...
class SpiMasterHarness:
    ...
    def _make_dut(self):
        if self.backend == "local":
            return self._ls.SpiMasterPublic(
                spec_cfs_min=self.secrets.get("spec_cfs_min"),
                spec_hold_ss=self.secrets.get("spec_hold_ss"),
                spec_txftlr_dflt=self.secrets.get("spec_txftlr_dflt"),
            )
    ...
    def _mk_verilator_dut(self):
        ...
        spec = importlib.util.spec_from_file_location("spi_master_vd", vd)
        mod = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(mod)
    ...
    def _mk_cov(self):
        ...
        spec = importlib.util.spec_from_file_location("spi_master_cs", cs)
        mod = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(mod)
        self._cs_mod = mod
        # spec_cfs_min drives the cfs_effective_min hidden-clamp bins
        return mod.CoverageSimulator(
            os.path.join(HERE, "dut", "coverage_meta.json"),
            spec_cfs_min=self.secrets.get("spec_cfs_min"),
        )
...
def run_episode(agent, secrets=None, max_steps=20000, interval=1000,
                backend="verilator"):
    h = SpiMasterHarness(secrets, backend=backend)
    state = h.reset()
    curve = []
...
if __name__ == "__main__":
    ...
    class RandAgent:
        ...
        def predict(self, coverage_state, step, max_steps):
            a = self.rng.uniform(0, 256, SpiMasterHarness.DIMS).astype(np.float32)
