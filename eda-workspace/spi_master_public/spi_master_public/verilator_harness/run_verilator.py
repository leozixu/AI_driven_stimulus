#!/usr/bin/env python3
"""
spi_master_public - Verilator harness runner
...
EXE = os.path.join(OBJ_DIR, "Vspi_m_top")
...
COV_KEYS = [
    ...
    "cov_frame_cnt", "cov_f_done", "cov_c_done", "cov_x_done",
    "cov_last_frame", "cov_tx_shift_en", "cov_rx_shift_en",
    "cov_load_tx_shift", "cov_load_tx_buffer", "cov_load_rx_buf",
    "cov_s3", "cov_txd", "cov_ss_in_n", "cov_ss_active",
    "cov_dfs", "cov_eff_dfs", "cov_cfs", "cov_eff_cfs", "cov_baudr",
    "cov_baud2", "cov_ndf", "cov_tmod", "cov_frf", "cov_scph", "cov_scpol",
    "cov_srl_test", "cov_ss_tgl_en", "cov_slv_oe", "cov_ser", "cov_tx_level",
    "cov_rx_level", "cov_tx_full", "cov_tx_empty", "cov_rx_full",
    "cov_rx_empty", "cov_tx_overflow", "cov_rx_overflow", "cov_rx_underflow",
    "cov_risr", "cov_isr", "cov_imr", "cov_ssi_txe_intr",
    "cov_ssi_mst_intr", "cov_s1", "cov_multi_mst_sync",
    "cov_s2", "cov_hold_ss_cnt", "cov_start_xfer", "cov_fsm_busy",
    "cov_fsm_sleep", "cov_ssi_busy", "cov_ssi_sleep", "cov_rx_push",
    "cov_tx_pop", "cov_rx_data", "cov_mwcr", "cov_spi0_control",
    "cov_spi1_control", "cov_txftlr", "reg_rdata",
]


class VerilatorHarness:
    """Drives the Verilator-compiled spi_m_top via a text protocol."""
    ...
    def __init__(self, spec_cfs_min: int = 8, spec_hold_ss: int = 4,
        ...
        self.spec_txftlr_dflt = spec_txftlr_dflt
        self._ensure_built()
    ...
    def run_actions(self, actions: List[Dict]) -> List[Dict]:
        ...
        for a in actions:
            ...
            addr = int(a.get("reg_addr", 0)) & 0x3F
        ...
        p = subprocess.run(
            [EXE, str(self.spec_cfs_min), str(self.spec_hold_ss),
...
def run_verilator_simulation(actions: List[Dict],
                             spec_cfs_min: int = 8, spec_hold_ss: int = 4,
                             spec_txftlr_dflt: int = 1) -> Dict:
    """Run a full simulation and return per-cycle signals."""
    harness = VerilatorHarness(spec_cfs_min, spec_hold_ss, spec_txftlr_dflt)
...
def run_and_save_curve(actions: List[Dict], interval: int = 1000,
    ...
    signals = run_verilator_simulation(actions, spec_cfs_min, spec_hold_ss,
                                       spec_txftlr_dflt)["signals"]
    total = len(signals)
    if total == 0:
        return []
...
if __name__ == "__main__":
    ...
    A = [{"rst_n": 0}, {"rst_n": 1},
         {"reg_we": 1, "reg_addr": 0x0, "reg_wdata": 0x0000_2807},  # dfs=7,frf=0,scph=0,tmod=2,srl=1 -> 8-bit frame
         {"reg_we": 1, "reg_addr": 0x4, "reg_wdata": 0x1},          # SER slave 0
         {"reg_we": 1, "reg_addr": 0x5, "reg_wdata": 0x8},          # BAUDR 8
         {"reg_we": 1, "reg_addr": 0x6, "reg_wdata": 0x0},          # TXFTLR 0
         {"reg_we": 1, "reg_addr": 0x2, "reg_wdata": 0x1},          # SSIENR 1 (enable before DR push: tx_push is gated by ssienr)
         {"reg_we": 1, "reg_addr": 0x18, "reg_wdata": 0xFF}]        # DR push
    A += [{"ss_in_n": 1}] * 300

    res = run_verilator_simulation(A)
    busy = [s for s in res["signals"] if s.get("cov_ssi_busy")]
    print(f"total cycles: {res['total_cycles']}, busy cycles: {len(busy)}")
    if busy:
